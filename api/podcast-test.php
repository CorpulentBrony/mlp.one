<?php
	...
	const CHANNEL_URL = "https://www.youtube.com/watch?list=" . YOUTUBE_PLAYLIST_ID;
	...
	function generateItems(\XMLWriter $xml, array $objectList, array $tagsMap): \XMLWriter {
		foreach ($objectList as $index => $object) {
			...
			xmlAddElementInline($xml, "title", [], rawurldecode(pathinfo($object["Key"], PATHINFO_FILENAME)));
			xmlAddElementInline($xml, "link", [], $youtubeUrl);
			// xmlAddElementInline($xml, "description", [], atob($tagsMap[$index]->get("description")));
			xmlAddElementInline($xml, "description", [], getDescription($tagsMap[$index]));
	...
	\GuzzleHttp\Promise\coroutine(function () use ($s3): \Generator {
		...
		// $xml->writePI("xml-stylesheet", "type=\"text/xsl\" href=\"https://mlp.one/api/podcast-sitemap.xsl\"");
		xmlAddElement($xml, "rss", ["xmlns:atom" => "http://www.w3.org/2005/Atom", "xmlns:itunes" => "http://www.itunes.com/dtds/podcast-1.0.dtd", "version" => "2.0"]);
		$xml->startElement("channel");
		xmlAddElementInline($xml, "title", [], CHANNEL_TITLE);
		xmlAddElementInline($xml, "link", [], CHANNEL_URL);
