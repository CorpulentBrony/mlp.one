...
<html itemid="<?= $this->episode->getGuid() ?>" itemscope itemtype="http://schema.org/WebPage" lang="en" prefix="og: http://ogp.me/ns# fb: http://ogp.me/ns/fb#" 🦄 🐎🐱>
	<head>
		...
		<script async defer id="material-components-web-script" src="https://unpkg.com/material-components-web@latest/dist/material-components-web.min.js"></script>
...
/*			.yt-video-container {
	...
	<body class="mdc-typography">
			...
			<div class="mdc-top-app-bar__row">
				...
				<section class="mdc-top-app-bar__section mdc-top-app-bar__section--align-end" role="toolbar">
					...
					<a class="mdc-top-app-bar__action-item" href="<?= $thisEpisodeNumber ?>.mp3" type="audio/mpeg">
						<img alt="Download MP3" aria-label="Download this episode in MP3 format" data-is-svg src="/material-design-icons/file/svg/production/ic_cloud_download_24px.svg" title="Download MP3" type="image/svg+xml">
					</a>
				</section>
			</div>
		</header>
		...
		<main>
			<article>
				<header>
					<nav aria-label="Navigate between episodes" role="navigation">
						<ul class="horizontal-icon-list">
							<li>
								<?php if (!is_null($previousEpisodeNumber)): ?>
									<a class="mdc-button mdc-button--raised" href="<?= strval($previousEpisodeNumber) ?>" rel="prev">
										<img alt="&lt;" aria-label="Go to previous episode" class="mdc-button__icon" data-is-svg src="/material-design-icons/navigation/svg/production/ic_arrow_back_24px.svg" title="Previous episode" type="image/svg+xml">
									</a>
								<?php endif; ?>
							</li>
							<li>
								<a class="mdc-button mdc-button--raised" href="/" rel="index">
									<img alt="Episode List" aria-label="Go to list of episodes" class="mdc-button__icon" data-is-svg src="/material-design-icons/action/svg/production/ic_view_list_24px.svg" title="List of episodes" type="image/svg+xml">
								</a>
							</li>
							<li>
								<?php if (!is_null($nextEpisodeNumber)): ?>
									<a class="mdc-button mdc-button--raised" href="<?= strval($nextEpisodeNumber) ?>" rel="next">
										<img alt="&gt;" aria-label="Go to next episode" class="mdc-button__icon" data-is-svg src="/material-design-icons/navigation/svg/production/ic_arrow_forward_24px.svg" title="Next episode" type="image/svg+xml">
									</a>
								<?php endif; ?>
							</li>
						</ul>
				...
				<footer>
					<nav>
						<ul aria-label="This episode is also available in the following data formats" class="mdc-chip-set" role="navigation" title="More data formats for this episode">
							<?=
								array_reduce($this->getAllRequestTypes(), function(string $links, array $typeDescriptor): string {
									return "{$links}<li><a class=\"mdc-chip\" href=\"{$typeDescriptor["url"]}\" type=\"{$typeDescriptor["mimeType"]}\"><span class=\"mdc-chip__text\">{$typeDescriptor["extension"]}</span></a></li>";
								}, "")
							?>
						</ul>
					</nav>
			...
			<article class="mdc-card">
				...
				<footer class="mdc-card__actions">
					...
					<nav class="mdc-card__action-icons">
						<?php if (!is_null($previousEpisodeNumber)): ?>
							<a class="mdc-button mdc-card__action mdc-card__action--icon" href="<?= strval($previousEpisodeNumber) ?>" rel="prev">
						...
						<?php endif; ?>
						<?php if (!is_null($nextEpisodeNumber)): ?>
							<a class="mdc-button mdc-card__action mdc-card__action--icon" href="<?= strval($nextEpisodeNumber) ?>" rel="next">
						...
						<?php endif; ?>
						<i class="mdc-ripple-surface mdc-card__action mdc-card__action--icon">
